output$diagram_algo <- renderGrViz({
  ...
  pcr_nos <- dengue_dta_filt() %>% filter(pcr_result %in% c("No sample", "No Sample")) %>% nrow()
  ...
  ns1_nos <- dengue_dta_filt() %>% filter(elisa_ns1_test_result %in% c("No sample", "No Sample")) %>% nrow()
  ...
  igm_nos <- dengue_dta_filt() %>% filter(elisa_ig_m_test_result %in% c("No sample", "No Sample")) %>% nrow()
